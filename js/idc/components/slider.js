  ...
  var Slider = function(selector, options) {
    ...
    var element,
      ...
      TEMPLATES = {
        ...
        prevButton: '<a href="#" class="prev-btn icon-arrow-left"></a>',
        nextButton: '<a href="#" class="next-btn icon-arrow-right"></a>'
      };
    ...
    function addEventListeners() {
      if (idc.utils.touch() || SETTINGS.forceTouch) {
        viewport.addEventListener(idc.Input.START, startHandler, false);
        viewport.addEventListener(idc.Input.MOVE, moveHandler, false);
        viewport.addEventListener(idc.Input.END, endHandler, false);
      }
      ...
      if (SETTINGS.arrows) {
        prevBtn.addEventListener(idc.Input.CLICK, prevBtn_clickHandler, false);
        nextBtn.addEventListener(idc.Input.CLICK, nextBtn_clickHandler, false);
      }
      
      if (!idc.utils.touch()) {
        document.body.addEventListener(idc.Input.END, releaseDragging, false);
        
        if (SETTINGS.showPager) {
          pager.find('a').off(idc.Input.CLICK).on(idc.Input.CLICK, pager_clickHandler);
        }
      }
    }
    ...
    function removeNavigation() {
      ...
      if (idc.utils.touch() || SETTINGS.forceTouch) {
        ...
        viewport.removeEventListener(idc.Input.START, startHandler, false);
        viewport.removeEventListener(idc.Input.MOVE, moveHandler, false);
        viewport.removeEventListener(idc.Input.END, endHandler, false);
      }
      ...
      if (!idc.utils.touch() && SETTINGS.forceTouch) {
        document.body.removeEventListener(idc.Input.END, releaseDragging, false);
      }

      // remove arrows
      if (SETTINGS.arrows && prevBtn && nextBtn) {
        prevBtn.removeEventListener(idc.Input.CLICK, prevBtn_clickHandler, false);
        nextBtn.removeEventListener(idc.Input.CLICK, nextBtn_clickHandler, false);
        $(prevBtn).remove();
        $(nextBtn).remove();
      }
    }
    ...
    function transitionEndHandler() {
      ...
      var pos = idc.utils.getTranslateCoordinate(itemsWrapper.style[Prefixr.transform], 'x');
